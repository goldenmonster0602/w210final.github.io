...
from bokeh.models import ColumnDataSource, Range1d, LinearAxis, HoverTool, Title, DatetimeTickFormatter
...
for app_name in app_names:
    ...
    final_versions_dict[app_name] = np.array(final_versions)
...
def print_relevant_reviews(distances, ratings, versions, reviews, n=10):
    ...
    for i in range(len(top_indices)):
        ...
        text += " (Rating: "+str(int(rating))+"/5 stars, version: "+version+")\n"
...
final_versions=final_versions_dict[initial_app_name]
r = 0.5
...
printed_reviews_text = print_relevant_reviews(distances, final_ratings, final_versions, final_reviews_unprocessed, n=10)
...
# Set up data
datestrings = [str(d)[5:10]for d in np.array(unique_dates, dtype=np.datetime64)]
...
def update_data():
    ...
    final_dates = final_dates_dict[app_name]
    final_versions = final_versions_dict[app_name]
    ...
    if start_rating != 1 or end_rating != 5:
        ...
        final_versions[ratings_indeces]
    ...
    if start_date != min_date or end_date != max_date:        
        ...
        print_versions = final_versions[review_indices]
    ...
    reviews.text = print_relevant_reviews(print_distances,print_ratings,print_versions, print_reviews_unprocessed, n=10)
...
#app_title = PreText(text="EMERGENT", width=col1_width, height = 300, text_font_size=20, text_font_style="bold")
